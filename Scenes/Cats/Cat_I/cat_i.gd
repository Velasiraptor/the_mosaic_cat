...
@onready var sprite_cat_i = %Sprite_Cat_I
@onready var collision_touch = %Collision_touch

@onready var cat_i_animation = %Cat_I_animation
@onready var collision_skeleton = %Collision_skeleton
...
func _ready():
	...
	sprite_cat_i.play("idle_classic")
...
func _set_drag_pc():
	...
	if dragging:
		var mousepos = get_viewport().get_mouse_position()
		self.global_position = get_global_mouse_position() - Vector2(44, 0) + Vector2(0, -64) #для телефона
...
func _on_input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed and not dragging:
			...
			cat_i_animation.visible = true
			cat_i_animation.play()
			
			if abs(position.x - get_parent().position.x) <= 400 and abs(position.y - get_parent().position.y) <= 400:
				get_tree().call_group("Spawner_Cat_I", "count_cat_minus") # Вычитаем надпись у спаунера
...
func _on_random_timer_blinks_timeout(): #Таймер моргания
	sprite_cat_i.play("blinks_classic")
	random_timer_blinks.start_random()
func _on_random_timer_meow_timeout(): #Таймер мяукания
	random_timer_blinks.stop()
	sprite_cat_i.play("meow_classic")
