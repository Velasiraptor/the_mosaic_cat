...
@onready var sprite_cat_z = %Sprite_Cat_Z
@onready var collision_touch = %Collision_touch

@onready var cat_z_animation = %Cat_Z_animation
...
func _on_input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed and not dragging:
			...
			#анимация
			sprite_cat_z.visible = false
			cat_z_animation.visible = true
			cat_z_animation.play()
			
			if abs(position.x - get_parent().position.x) <= 400 and abs(position.y - get_parent().position.y) <= 400:
				get_tree().call_group("Spawner_Cat_Z", "count_cat_minus") # Вычитаем надпись у спаунера
			collision_touch.scale = Vector2(8, 8)
			sprite_cat_z.z_index = 1
			cat_z_animation.z_index = 1
...
func not_dragging():
	...
	sprite_cat_z.z_index = 0
	cat_z_animation.z_index = 0
	if count_tile_in_cat != 0:
		...
		get_tree().call_group("Spawner_Cat_Z", "count_cat_plus") # Прибавляем надпись у спаунера
		get_tree().call_group("Spawner_Cat_Z", "check_pickable_cats")
		rotation_degrees = 0
		collision_touch.scale = Vector2(1.8, 1)
...
func _on_random_timer_blinks_timeout(): #Таймер моргания
	sprite_cat_z.play("blinks_classic")
	random_timer_blinks.start_random()
